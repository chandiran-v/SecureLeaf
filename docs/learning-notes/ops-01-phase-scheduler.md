...
| (recovery re-run) push rejected: `refusing to allow a Personal Access Token to create or update workflow … without workflow scope` | Claude's branch was cut from the target **before** `phase-scheduler.yml` was edited. For a *new* branch GitHub checks the pushed tree against the default branch, so the older workflow file looked like a workflow change. Claude never touched `.github/` | `publish.sh` rebases new work onto the target's current tip before pushing (a conflict parks the item). We did **not** grant the token `workflow` scope | Keep branches current instead of widening credentials. And "what did this commit change" depends on what you compare against |
