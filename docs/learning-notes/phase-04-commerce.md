...
4. ~~**The integration suite isn't wired into CI** (no Failsafe)~~ — **Fixed in [Phase 05A](phase-05-secure-viewer.md)**: `maven-failsafe-plugin` is now bound to `integration-test`/`verify`, so `mvn verify` actually runs every `*IT.java` in the project. That run surfaced the same three pre-existing failures this note had already found by hand (`PreviewControllerIT.previewBytes_differ…`, `ProcessingPipelineIT.processAsync_success`, `MarketplaceQueryIT.search_usesTheFtsIndex`) — all fixed as part of Phase 05A so `mvn verify` passes clean. Testcontainers is still pinned at 1.19.8; revisit if a future Docker Engine version stops working with it.
